...
private:
  ...
  const string paramName[20] = {"LEVEL", "MIX", "F.BACK", "HiCUT", "LoCUT", "HiDUMP", "BOOST"};
  enum paramName {LEVEL, MIX, FBACK, HICUT, LOCUT, HIDUMP, BOOST};
  float param[20] = {1, 1, 1, 1, 1, 1, 1};
  const int16_t paramMax[20] = {100,100, 99,100,100,100,100};
  ...
  signalSw bypassIn, bypassOutL, bypassOutR, boostL, boostR;
  delayBufF del[10];
  lpf lpfIn, lpfFB[4];
  hpf hpfOutL, hpfOutR;
...
public:
  ...
  void setParam() override
  {
    ...
    count = (count + 1) % 14; // 負荷軽減のためパラメータ計算を分散させる
    switch(count)
    {
      case 0:
  ...
  void process(float xL[], float xR[]) override
  {
    ...
    for (uint16_t i = 0; i < BLOCK_SIZE; i++)
    {
      ...
      fxL = boostL.process(fxL, param[BOOST] * fxL, exOn); // BOOST
      fxR = boostR.process(fxR, param[BOOST] * fxR, exOn); // BOOST

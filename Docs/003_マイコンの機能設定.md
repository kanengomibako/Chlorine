...
~~2025年4月7日時点の最新STM32CubeMXソフトウェア（6.14）では、SAIの設定を行うことができません。iocファイルをテキストエディタで開き、「MxCube.Version=6.13.0」「MxDb.Version=DB.6.0.130」と編集すると旧バージョンになります。開くときに新しいバージョンに移行する（Migrate）かダイアログが出るので、旧バージョンのままの Continue を選択します。~~2026年4月8日時点の最新STM32CubeMXソフトウェア（6.17.0）では、修正されています。
